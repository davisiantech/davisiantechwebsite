document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateAnnouncement() {
        ...
        if (activeAnnouncement) {
            ...
            // Keep the announcement in normal page flow and position the header below it.
            const updateLayout = () => {
    ...
    // Give the hours section clear context during and outside the operating season.
    function renderSeasonContext() {
        if (typeof SEASON_DATA === 'undefined' || !SEASON_DATA) return;

        const title = document.getElementById('season-status-title');
        const copy = document.getElementById('season-status-copy');
        const hoursLabel = document.getElementById('hours-label');
        if (!title || !copy || !hoursLabel) return;
        ...
        if (now < openDate) {
            const openText = openDate.toLocaleDateString('en-US', { month: 'long', day: 'numeric' });
            title.innerHTML = 'Closed for now.<br>Opening ' + openText + '.';
            copy.textContent = 'Gary’s is getting ready for another summer. Check the dates here as opening day gets closer.';
            hoursLabel.textContent = 'Upcoming in-season hours';
            document.body.classList.add('season-closed');
        } else if (now > closeDate) {
            title.innerHTML = 'Closed for the season.<br>See you next summer.';
            copy.textContent = 'Thanks for another wonderful season at Gary’s. We look forward to welcoming everyone back next summer.';
            hoursLabel.textContent = 'Regular in-season hours';
            document.body.classList.add('season-closed');
        } else {
            title.innerHTML = 'A summer day<br>well spent.';
            copy.textContent = 'Come for a round, stay for a Jiffy Pup, and leave a little room for ice cream.';
            hoursLabel.textContent = 'Current hours and season dates';
            document.body.classList.remove('season-closed');
        }
    }
